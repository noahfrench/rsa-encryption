...
public class RSA {
	...
	// If p == q, recalculate q until it's something else!
	while (p == q) {
	    q = new BigInteger(bitLength, random);
	}
	
	BigInteger n = p.multiply(q);
	int bitLengthE = n.bitLength() - 1;
	BigInteger e = new BigInteger(bitLengthE, 100, random);
	BigInteger bigOne = BigInteger.valueOf(1);
	BigInteger pq = (p.subtract(bigOne)).multiply((q.subtract(bigOne)));	
	BigInteger d = e.modInverse(pq);

	RSAKey theKey = new RSAKey();
	theKey.d = d;
	theKey.e = e;
	theKey.n = n;
	theKey.bitLength = bitLength;
    ...
    public static CipherText encrypt (RSAKey key, String plainText) throws Exception {
        // your code here; a dummy return statement is put below to allow this to compile
	byte[] asciiText = plainText.getBytes();
	ArrayList<BigInteger> retEncryptedBlocks = new ArrayList<BigInteger>();
	BigInteger currentChar = new BigInteger("0");
	
        for (int i = 0; i < asciiText.length; i++) {
            currentChar = BigInteger.valueOf(asciiText[i]);
            BigInteger currentBlock = currentChar.modPow(key.e, key.n);
            retEncryptedBlocks.add(currentBlock);
	}
	
	CipherText retCipher = new CipherText();
	retCipher.encryptedBlocks = retEncryptedBlocks;
	retCipher.fileLength = plainText.length();
	retCipher.blockLength = 1;

	/*
	for (int i = 0; i < retCipher.encryptedBlocks.size(); i++) {
	    System.out.println(retCipher.encryptedBlocks.get(i));
	}
	System.out.println(plainText);
	System.out.println("fileLength: " + retCipher.fileLength);
	System.out.println("blockLength: " + retCipher.blockLength)
	*/
	...
	String decrypted = new String();
	    ...
	    BigInteger decBigInt = cipherText.encryptedBlocks.get(i).modPow(key.d, key.n);
	    byte[] currentAscii =decBigInt.toByteArray();
	    String backToChar = new String(currentAscii);
	    decrypted = decrypted + backToChar;
	}
	...
	// Calculate the prime factors of n (p and q)
	for (long i = 2; i < key.n.longValue(); i++) {
	    if (key.n.longValue() % i == 0) {
		p = key.n.longValue() / i;
		foundP = true;
		break;
	    }
	...
	BigInteger d = key.e.modInverse(pqBig);
	...
	byte[] byties = plainText.getBytes();
	MessageDigest digester = MessageDigest.getInstance(hashAlgorithm);
	//byte[] digested = digester.digest(byties);
	digester.update(byties);
	String digestedString = convertHash(digester.digest());

	RSAKey tempKey = key;
	tempKey.e = key.d;

	CipherText encrypted = encrypt(tempKey, digestedString);
    ...
    public static boolean checkSign (RSAKey key, String plainText, CipherText signature) throws Exception {
        ...
        RSAKey tempKey = key;
        tempKey.d = key.e;
        String decrypted = decrypt(tempKey, signature);
        byte[] byties = plainText.getBytes();
        MessageDigest digester = MessageDigest.getInstance(hashAlgorithm);
        byte[] digested = digester.digest(byties);
        String digestedString = convertHash(digested);

        return decrypted.equals(digestedString);
    }
